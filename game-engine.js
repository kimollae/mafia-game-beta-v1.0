...
function createGameState(roomData) {
  ...
  return {
    ...
    firstDayNoExec: true, // 첫날 낮 처형 없음
    nightActions: {}, dayActions: {}, voteLog: {},
    soldierShield: {}, doctorSelfUsed: {},
    gamblerSuccess: {}, gamblerBetCache: {}, gamblerBetTarget: {}, lawyerProtect: {},
    actionHistory: {},
    // ★ [Q9] 방장 관리
    host: roomData.host,
    hostTransferAt: null,
...
function processNight(gameState) {
  // ★ deepClone 전에 null 필드 보완 (Firebase는 빈 객체를 null로 저장)
  if (!gameState.gamblerBetCache) gameState.gamblerBetCache = {};
  if (!gameState.gamblerSuccess)  gameState.gamblerSuccess  = {};
  if (!gameState.soldierShield)   gameState.soldierShield   = {};
  if (!gameState.doctorSelfUsed)  gameState.doctorSelfUsed  = {};
  ...
  // deepClone 후에도 재확인
  gs.gamblerBetCache = gs.gamblerBetCache || {};
  gs.gamblerSuccess  = gs.gamblerSuccess  || {};
  gs.soldierShield   = gs.soldierShield   || {};
  gs.doctorSelfUsed  = gs.doctorSelfUsed  || {};
  gs.actionHistory   = gs.actionHistory   || {};

  const isAlive = uid => assign[uid]?.alive === true;
  const roleOf  = uid => assign[uid]?.role;
  const teamOf  = uid => assign[uid]?.team;

  // ★ [Q1] 첫날 밤(day=1): 살해만 불가, 포섭/조사/보조 모두 허용
  const isFirstNight = (gs.day === 1);
  if (isFirstNight) {
    Object.keys(actions).forEach(uid => {
      const act = actions[uid];
      if (act && act.action === 'kill') {
        delete actions[uid]; // 살해만 제거
      }
    });
  }
  ...
  if (isRecruiting && bossAction?.target1) {
    ...
    if (!isAlive(tuid) || teamOf(tuid) !== 'citizen') {
      ...
      results[tuid]    = { type:'recruited', msg:'당신은 포섭되었습니다. 오늘 밤 능력은 무효화됩니다.', newRole: assign[tuid]?.role || 'KILLER' };
      ...
      if (gs.recruitUsed === 1) {
        ...
        if (jsUID) {
          ...
          results[jsUID] = { type:'inherit', msg:'직업 승계: ' + ROLES[originalRole].nameKo, newRole: originalRole };
        }
      }
    }
  }
  ...
  const lawyerMasked  = new Set(); // 변호사 조사위장 제거 (처형면제로 변경)

  Object.entries(actions).forEach(([uid, act]) => {
    if (!isAlive(uid) || !act?.target1) return;
    const role = roleOf(uid);
    if (role === 'DOCTOR') {
      protectedUIDs.add(act.target1); // 자기 자신 포함 보호 가능
    }
    // ★ [Q3] 포섭은 의사/군인 보호 무용 → guardedMap/protectedUIDs는 살해에만 적용
    if (role === 'SOLDIER') guardedMap[act.target1] = uid;
    if (role === 'LAWYER') {
      // 처형 면제: 다음날 낮에 이 사람은 처형 면제
      if (!gs.lawyerProtect) gs.lawyerProtect = {};
      gs.lawyerProtect[uid] = act.target1;
      results[uid] = { type:'lawyer_protect', target: act.target1,
        msg:'처형 보호 완료. 내일 낮 처형에서 보호됩니다.' };
    }
  });
  ...
  Object.entries(actions).forEach(([uid, act]) => {
    ...
    if (role === 'GAMBLER' && act.prediction) {
      ...
      if (act.target1) {
        if (!gs.gamblerBetTarget) gs.gamblerBetTarget = {};
        gs.gamblerBetTarget[uid] = act.target1;
      }
    }
  });
  ...
  Object.keys(assign).forEach(uid => {
    ...
    delete assign[uid].killedBy; delete assign[uid].killedDay;
  ...
  Object.entries(gs.gamblerBetCache).forEach(([uid, prediction]) => {
    ...
    if (prediction === 'death') {
      const betTarget = gs.gamblerBetTarget?.[uid];
      const hit = betTarget && deadUID && betTarget === deadUID;
      if (hit) {
        gs.gamblerSuccess[uid] = (gs.gamblerSuccess[uid] || 0) + 1;
        const cnt = gs.gamblerSuccess[uid];
        const bonus = getBonusInfo(gs, uid);
        results[uid] = { type:'gambler_win', successCount:cnt, bonus,
          msg:'사망 예측 성공! (' + cnt + '/3)' + (bonus?' '+ROLES[bonus.role].nameKo+' 직업 정보 획득':'') };
      } else if (!deadUID) {
        results[uid] = { type:'gambler_fail', msg:'이 밤 사망자 없음 — 사망 예측 실패' };
      } else {
        results[uid] = { type:'gambler_fail', msg:'예측 빗나감 — 사망 예측 실패' };
      }
      delete gs.gamblerBetTarget?.[uid];
    }
    ...
    if (prediction === 'peace') {
      if (!deadUID) {
        ...
        const bonus = getBonusInfo(gs, uid);
        results[uid] = { type:'gambler_win', successCount:cnt, bonus,
          msg:'평화 예측 성공! (' + cnt + '/3)' + (bonus?' '+ROLES[bonus.role].nameKo+' 직업 정보 획득':'') };
      } else {
...
function processVote(gameState) {
  ...
  // ★ [Q1] 첫날 낮 처형 없음 — isFirstDay 플래그로 관리
  // processNight에서 day가 1→2로 바뀌므로 day===2가 첫번째 낮
  // 하지만 firstDayNoExec 플래그가 더 안전
  const isFirstDay = gs.firstDayNoExec === true;
  ...
  // firstDayNoExec는 첫 투표 후 해제
  gs.firstDayNoExec = false;

  const results = {};

  if (executed) {
    // ★ 변호사 처형 면제 체크
    const lawyerProtectTarget = Object.values(gs.lawyerProtect || {}).find(t => t === executed);
    if (lawyerProtectTarget) {
      const lawyerUID = Object.keys(gs.lawyerProtect || {}).find(k => gs.lawyerProtect[k] === executed);
      results[executed] = { type:'lawyer_immune', msg:'변호사의 보호로 처형이 면제되었습니다.' };
      if (lawyerUID) results[lawyerUID] = { type:'lawyer_protect_success', target: executed, msg:'보호 성공 — 처형 면제 발동' };
      gs.lawyerProtect = {}; // 1회 사용 후 초기화
      executed = null;
    }
    // ★ [Q4] 정치인 면제
    if (executed && assign[executed]?.role === 'POLITICIAN') {
      ...
      if (executed === gs.clientUID && gs.lawyerCondition === 'client_alive') {
        ...
        const lawyerUID2 = Object.keys(assign).find(u => assign[u].role === 'LAWYER' && isAlive(u));
        if (lawyerUID2) results[lawyerUID2] = { type:'client_executed', msg:'의뢰인이 처형되었습니다. 패배 조건 발동.' };
      }
    }
  }
...
function checkWinCondition(gs) {
  ...
  // ★ 마피아 승리: 시민 + 중립 전부 제거
  const aliveNonMafia = aliveCitizen + aliveNeutral;
  const mafiaWins = aliveMafia > 0 && aliveNonMafia === 0;

  if (mafiaWins) {
    return { winner:'mafia', reason:'시민 및 중립 세력 전멸', lawyerWin:resolveLawyerWin(gs,'mafia_wins'), gamblerWin:resolveGamblerWin(gs) };
  }
  if (aliveMafia === 0) {
...
// ── 도박꾼 최종 승리 ─────────────────────────────────────────
function getBonusInfo(gs, uid) {
  const cnt = gs.gamblerSuccess?.[uid] || 0;
  if (cnt < 1) return null;
  // 살아있는 플레이어 중 랜덤 1명 직업 힌트
  const alive = Object.entries(gs.assignments || {})
    .filter(([u, p]) => p.alive && u !== uid)
    .map(([u, p]) => ({ uid: u, role: p.role }));
...
// ────────────────────────────────────────────────────────────
const GameDB = {
  // Firebase undefined 필드 제거 (Firebase는 undefined 불허)
  _cleanUndefined(obj) {
    if (typeof obj !== 'object' || obj === null) return obj;
    Object.keys(obj).forEach(k => {
      if (obj[k] === undefined) delete obj[k];
      else if (typeof obj[k] === 'object') MafiaEngine.GameDB._cleanUndefined(obj[k]);
    });
    return obj;
  },
  ...
  async startGame(db, roomId) {
    ...
    const hostUID = roomVal.host; // 방장 UID
    const gs      = createGameState(roomVal);

    // ★ 1. playerRoles 전부 먼저 병렬 작성 (gameState보다 먼저!)
    const writes = {};
    for (const [uid, info] of Object.entries(gs.assignments)) {
      ...
      writes['playerRoles/' + roomId + '/' + uid] = roleData;
    }
    await db.ref().update(writes); // 병렬 원자적 작성

    // ★ 2. 그 다음 gameState 작성 → 이때 클라이언트 구독 발동
    //    → 구독 발동 시 playerRoles는 이미 다 있음
  ...
  async processNightActions(db, roomId) {
    ...
    // undefined 필드 제거 후 저장
    const cleanGS = JSON.parse(JSON.stringify(newGS, (k,v) => v === undefined ? null : v));
    await db.ref('rooms/' + roomId + '/gameState').set(sanitize(cleanGS));
  ...
  async processDayActions(db, roomId) {
    ...
    await db.ref('rooms/' + roomId + '/gameState').set(sanitize(newGS));
  ...
  async processVoteResults(db, roomId) {
    ...
    if (!snap.exists()) throw new Error('gameState 없음');
    const gs = snap.val();
    if (!gs.phase || gs.phase !== 'vote') {
      console.warn('[processVote] 잘못된 phase:', gs.phase);
      return { gameState: gs, results: {}, executed: null };
    }
