...
const SkillFX = (() => {
  ...
  const effects = {
    ...
    KILLER(targetUID) {
      ...
      `, 1500);
      // 화면 흔들림 (app 없어도 body로 fallback)
      const shakeTarget = document.getElementById('app') || document.body;
      shakeTarget.style.animation = 'fxShake 0.4s ease';
      setTimeout(() => shakeTarget.style.animation = '', 500);
      SoundFX.play('kill');
    },
