...
const BotEngine = (() => {
  ...
  async function runBotAction(db, roomId, uid, gs, clientUID = null) {
    if (!gs?.assignments?.[uid]) return;
    const assignment = gs.assignments[uid];
    if (!assignment.alive) return;
    ...
    try {
      if (gs.phase === 'night') {
        ...
        const action = role === 'LAWYER'
          ...
          : decisionFn(gs, uid);
        if (!action) return;
        console.log('[BOT]', uid, role, 'night action:', action.action);
        await db.ref('rooms/' + roomId + '/gameState/nightActions/' + uid).set(action);

      } else if (gs.phase === 'day') {
        if (role === 'SHAMAN') {
          if (gs.dayActions?.[uid]) return;
          const deadPlayers = Object.keys(gs.assignments).filter(u => !gs.assignments[u].alive);
  ...
  async function manageBots(db, roomId) {
    try {
      ...
      for (const [uid, player] of Object.entries(players)) {
        ...
        if (!gs.assignments[uid]?.alive) continue;

        const clientUID = gs.assignments[uid]?.role === 'LAWYER' ? gs.clientUID : null;
        promises.push(runBotAction(db, roomId, uid, gs, clientUID));
      }

      if (promises.length > 0) {
        console.log('[BOT] managing', promises.length, 'bots, phase:', gs.phase);
        await Promise.all(promises);
      }
    } catch(e) {
      console.warn('[manageBots error]', e.message);
    }
  }

  // ── 타이머 종료 시 미제출 강제 처리 ─────────────────────
  async function fillMissingActions(db, roomId) {
    // manageBots가 모두 처리하므로 위임
    await manageBots(db, roomId);
  }
