...
const SoundFX = (() => {
  ...
  const synth = {
    ...
    // ★ 밤 BGM — 긴박한 다크 스릴러 (노이즈 완전 제거)
    bgmNight() {
      bgmGain = ctx.createGain();
      bgmGain.gain.value = 0;
      bgmGain.gain.linearRampToValueAtTime(0.20, ctx.currentTime + 2);
      bgmGain.connect(compressor);

      const BPM = 70; // 낮보다 느리지만 긴박한 템포
      const beat = 60 / BPM;
      ...
      const kickFn = () => {
        ...
        const t2 = ctx.currentTime;
        // 강박
        const k1 = ctx.createOscillator();
        const k1g = ctx.createGain();
        k1.type = 'sine';
        k1.frequency.setValueAtTime(80, t2);
        k1.frequency.exponentialRampToValueAtTime(28, t2 + 0.2);
        k1g.gain.setValueAtTime(0.5, t2);
        k1g.gain.exponentialRampToValueAtTime(0.001, t2 + 0.3);
        k1.connect(k1g); k1g.connect(bgmGain);
        k1.start(t2); k1.stop(t2 + 0.35);
        // 약박 (0.2박 후)
        setTimeout(() => {
          if (!bgmGain) return;
          ...
          k2.type = 'sine';
          k2.frequency.setValueAtTime(60, t3);
          k2.frequency.exponentialRampToValueAtTime(25, t3 + 0.15);
          k2g.gain.setValueAtTime(0.3, t3);
          k2g.gain.exponentialRampToValueAtTime(0.001, t3 + 0.2);
          k2.connect(k2g); k2g.connect(bgmGain);
          k2.start(t3); k2.stop(t3 + 0.25);
        }, beat * 300);
      };
      const kickId = setInterval(kickFn, beat * 1000);
      bgmNodes.push({ stop: () => clearInterval(kickId) });
      ...
      // ② 불안한 베이스 멜로디 (단조 반복)
      const bassSeq = [55, 55, 49, 55, 52, 49, 44, 49];
      let bIdx = 0;
      const bassFn = () => {
        if (!bgmGain) return;
        const t2 = ctx.currentTime;
        ...
        bo.type = 'triangle';
        bo.frequency.value = bassSeq[bIdx % bassSeq.length];
        bIdx++;
        bg.gain.setValueAtTime(0, t2);
        bg.gain.linearRampToValueAtTime(0.15, t2 + 0.02);
        bg.gain.setValueAtTime(0.15, t2 + beat * 0.75);
        bg.gain.exponentialRampToValueAtTime(0.001, t2 + beat * 0.95);
      ...
      // ③ 긴장감 멜로디 (간헐적 단음 — 순수 사인)
      const nightNotes = [220, 196, 185, 165, 185, 196, 220, 247];
      let nIdx = 0;
      const scheduleNote = () => {
        if (!bgmGain) return;
        const t2 = ctx.currentTime;
        ...
        ng.gain.linearRampToValueAtTime(0.10, t2 + 0.015);
        ng.gain.exponentialRampToValueAtTime(0.001, t2 + 1.8);
        no.connect(ng); ng.connect(bgmGain);
        no.start(t2); no.stop(t2 + 2.0);
      ...
      [440, 554].forEach((freq, i) => {
        ...
        pg.gain.linearRampToValueAtTime(0.02 / (i + 1), ctx.currentTime + 3);
