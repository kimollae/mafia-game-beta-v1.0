...
const SoundFX = (() => {
  ...
  const synth = {
    ...
    // ★ 밤 BGM v4 — 리듬+멜로디 중심, 드론/패드 완전 제거
    bgmNight() {
      bgmGain = ctx.createGain();
      bgmGain.gain.value = 0;
      bgmGain.gain.linearRampToValueAtTime(0.22, ctx.currentTime + 1.5);
      bgmGain.connect(compressor);
      ...
      // ① 킥 — 두둠칫 패턴 (1박, 2.5박)
      const kickPattern = [0, beat * 1.5];
      const kickFn = () => {
        if (!bgmGain) return;
        kickPattern.forEach(offset => {
          const t2 = ctx.currentTime + offset;
          const ko = ctx.createOscillator();
          const kg = ctx.createGain();
          ko.type = 'sine';
          ko.frequency.setValueAtTime(90, t2);
          ko.frequency.exponentialRampToValueAtTime(30, t2 + 0.18);
          kg.gain.setValueAtTime(0.55, t2);
          kg.gain.exponentialRampToValueAtTime(0.001, t2 + 0.28);
          ko.connect(kg); kg.connect(bgmGain);
          ko.start(t2); ko.stop(t2 + 0.32);
        });
      };
      const kickId = setInterval(kickFn, beat * 2000);
      kickFn();
      bgmNodes.push({ stop: () => clearInterval(kickId) });
      ...
      const snareFn = () => {
        ...
        [beat, beat * 3].forEach(offset => {
          ...
          sg.gain.setValueAtTime(0.18, t2);
          sg.gain.exponentialRampToValueAtTime(0.001, t2 + 0.06);
          so.connect(sg); sg.connect(bgmGain);
          so.start(t2); so.stop(t2 + 0.08);
        });
      };
      const snareId = setInterval(snareFn, beat * 4000);
      snareFn();
      bgmNodes.push({ stop: () => clearInterval(snareId) });

      // ③ 베이스라인 — Am 단조 (triangle, 깔끔)
      const bassSeq = [55, 55, 49, 52, 44, 44, 49, 52];
      let bIdx = 0;
      const bassFn = () => {
        if (!bgmGain) return;
        const t2 = ctx.currentTime;
        ...
        bg.gain.linearRampToValueAtTime(0.18, t2 + 0.01);
        bg.gain.setValueAtTime(0.18, t2 + beat * 0.8);
        bg.gain.exponentialRampToValueAtTime(0.001, t2 + beat * 0.98);
        bo.connect(bg); bg.connect(bgmGain);
        bo.start(t2); bo.stop(t2 + beat + 0.01);
      };
      const bassId = setInterval(bassFn, beat * 1000);
      bassFn();
      bgmNodes.push({ stop: () => clearInterval(bassId) });

      // ④ 멜로디 — Am 단조 스케일 (sine, 간헐적)
      const melSeq = [220, 196, 220, 247, 220, 185, 165, 185];
      let mIdx = 0;
      const scheduleMel = () => {
        if (!bgmGain) return;
        const t2 = ctx.currentTime;
        ...
        const mg = ctx.createGain();
        mo.type = 'sine';
        mo.frequency.value = melSeq[mIdx % melSeq.length];
        mIdx++;
        mg.gain.setValueAtTime(0, t2);
        mg.gain.linearRampToValueAtTime(0.12, t2 + 0.01);
        mg.gain.exponentialRampToValueAtTime(0.001, t2 + beat * 1.8);
        mo.connect(mg); mg.connect(bgmGain);
        mo.start(t2); mo.stop(t2 + beat * 2);
        setTimeout(scheduleMel, (beat * 2 + Math.random() * beat) * 1000);
      };
      setTimeout(scheduleMel, 1200);
      bgmNodes.push({ stop: () => {} });
    },
